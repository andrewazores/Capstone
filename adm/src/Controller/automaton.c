...
void load_automaton(char* file_name,Automaton* g_automaton,int process_rank )
{
	...
	AutomatonTransition* transitions=(AutomatonTransition*) malloc(g_automaton->transitions_count*sizeof(AutomatonTransition));
	int tran_id=0;
	for(i=0;i<g_automaton->transitions_count;i++)
	{
		...
		tmp = strdup(line);
		char** pred_states_array=split_states(getfield(tmp, 3),'&',MAX_CONJUNCTS_PER_PREDICATE);
		int j;
...
int is_global_state_consistent_for_pred(int process_rank,int processes_count,Predicate* pred,GlobalState * global_state,int* inconsistent,int* pred_participants)
{
	int j ;
...
AutomatonState* advance_automaton(GlobalState* global_state,char* current_state_name,
		int processes_count,int process_rank,
		Automaton* g_automaton, int* automaton_params,int params_count )
{
	...
	for(i=0;i<g_automaton->transitions_count;i++)
	{
		...
		LOG_PRINT("C%i: advance_automaton, checking transition:%i from %s to %s",process_rank,tran.id,tran.from,tran.to);
		...
		int is_con=is_global_state_consistent_for_pred(process_rank,processes_count,pred,global_state,NULL,pred_participants);
		if(is_con==0)
		{
		...
		{
			LOG_PRINT("C%i: advance_automaton, transition from %s to %s not satisfied",process_rank,tran.from,tran.to);
		...
		int*automaton_params,int params_count)
{
	...
	int sat=global_state_satisfies_pred(global_state, pred,  unsatisfying_processes_boolean_array,process_rank,processes_count);
	...
	int is_con=is_global_state_consistent_for_pred(process_rank,processes_count,pred,global_state,inconsistent,pred_participants);
	...
	LOG_PRINT("C%i: could_be_satisfied_by, sat:%i, is_con:%i, unsat[%i]=%i",process_rank ,sat,is_con,process_rank,unsatisfying_processes_boolean_array[process_rank] );
	if(sat==1 && is_con==1)
	{
...
)
{
	...
	for(i=0;i<processes_count;i++)
	{
		LOG_PRINT("C%i: global_state->processes_states_array[%i]->variables.state_name_array[0]=%s",process_rank,i,global_state->processes_states_array[i]->variables.state_name_array[0]);
	...
	for(i=0;i<pred->conjunct_count;i++)
	{
		...
		if( satisfies(&(pred->conjuncts[i]),&(variables_array[pred->conjuncts[i].owner_processes[0]]),unsatisfying_processes_boolean_array,process_rank)==0)
		{
			satisfy= 0;
		}
...
int token_satisfies_pred(SendableToken *t,int process_rank)
{
	...
	for(j=0;j<t->predicates_count;j++)
	{
		...
		for(i=0;i<t->predicates[j].conjunct_count;i++)
		{
			...
			{
				pred_sat= 0;
				LOG_PRINT("C%i: token_satisfies_pred: token doesn't satisfy conjunct:%s",process_rank,t->predicates[j].conjuncts[i].conjunct_name);
			...
			{
				LOG_PRINT("C%i: token_satisfies_pred: token satisfies conjunct:%s",process_rank,t->predicates[j].conjuncts[i].conjunct_name);
...
int is_consistent3(GlobalState* global_state ,int processes_count, int * inconsistent_processes_boolean_array,int my_rank)
{
	int is_con = 1;
	// 1 -1 with me first
	...
	for (i = 0; i < processes_count; i++)
	{
		...
		vector_clock_compare res = compare_clocks(global_state->processes_states_array[i]->vector_clock,global_state->processes_states_array[my_rank]->vector_clock, processes_count);
		if (res != VC_CONCURRENT && res != VC_EQUAL)
		{
			is_con = 0;
	...
	for (i = 0; i < processes_count; i++)
	{
		...
		for (j = i+1; j < processes_count; j++)
		{
			...
			vector_clock_compare res = compare_clocks(global_state->processes_states_array[i]->vector_clock,global_state->processes_states_array[j]->vector_clock, processes_count);
			if (res != VC_CONCURRENT && res != VC_EQUAL)
			{
				is_con = 0;
				if (inconsistent_processes_boolean_array!=NULL)
...
int is_consistent2(GlobalState* global_state ,int processes_count, int * inconsistent_processes_boolean_array,int my_rank)
{
	int is_con=1;
	int i;
...
int is_consistent4(GlobalState* global_state ,int processes_count, int * inconsistent_processes_boolean_array,int my_rank) {
	int is_con = 1;
	int i;
...
int is_consistent(GlobalState* global_state  ,int processes_count, int * inconsistent_processes_boolean_array,int my_rank) {
	int is_con = 1;
	int i;
	...
	for (i = 0; i < processes_count; i++) {
		...
		vector_clock_compare res = compare_clocks(global_state->gcut, global_state->processes_states_array[my_rank]->vector_clock, processes_count);
		if (res != VC_CONCURRENT && res != VC_EQUAL)
			//if(local_state->vector_clock[i]  > gcut[i]) //the event knows something the global state doesn't know
		{
			is_con = 0;
