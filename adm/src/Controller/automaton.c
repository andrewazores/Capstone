...
void load_automaton(char* file_name, Automaton* g_automaton, int process_rank) {
	...
	AutomatonState* states = (AutomatonState*) malloc(
			g_automaton->states_count * sizeof(AutomatonState));
	int i;
	...
	for (i = 0; i < g_automaton->transitions_count; i++) {
		...
		for (j = 0; j < MAX_CONJUNCTS_PER_PREDICATE; j++) {
			...
			transition.pred.conjuncts[transition.pred.conjunct_count] =
					*conjunct_ptr;
...
Predicate* deparametrize_pred(Predicate* predicate,
		int* automaton_params_valuation, int params_count, int process_rank) {
	...
	for (i = 0; i < pred->conjunct_count; i++) {
		...
		strcpy(deparametrized_conjunct_name,
				predicate->conjuncts[i].conjunct_name);
		...
		for (j = 0; j < params_count; j++) {
			...
			strcpy(deparametrized_conjunct_name,
					str_replace(deparametrized_conjunct_name, pattern,
							replacer));
		...
		Conjunct* conjunct = get_conjunct_by_name(deparametrized_conjunct_name,
				process_rank);
		if (conjunct == NULL) {
			LOG_PRINT("C%i: deparametrizing conjunct %s NOT found",
					process_rank, deparametrized_conjunct_name);
			//exit(0);
		}
...
int is_global_state_consistent_for_pred(int process_rank, int processes_count,
		Predicate* pred, GlobalState * global_state, int* inconsistent,
		int* pred_participants) {
	int j;
		...
		for (k = j + 1; k < processes_count; k++) {
			if (pred_participants[j] == 1 && pred_participants[k] == 1) {
				vector_clock_compare res = compare_clocks(
						...
						global_state->processes_states_array[k]->vector_clock,
						processes_count);
...
AutomatonState* advance_automaton(GlobalState* global_state,
		char* current_state_name, int processes_count, int process_rank,
		Automaton* g_automaton, int* automaton_params, int params_count) {
	LOG_PRINT("C%i: advance_automaton", process_rank);
	int i;

	for (i = 0; i < g_automaton->transitions_count; i++) {
		AutomatonTransition tran = g_automaton->transitions[i];
		if (strcmp(tran.from, current_state_name) != 0
				|| strcmp(tran.from, tran.to) == 0) {
			continue;
		}

		LOG_PRINT(
				"C%i: advance_automaton, checking transition:%i from %s to %s",
				process_rank, tran.id, tran.from, tran.to);
		Predicate* pred = deparametrize_pred(&(tran.pred), automaton_params,
				params_count, process_rank);
		int* pred_participants = malloc(sizeof(int) * processes_count);
		memset(pred_participants, 0, sizeof(int) * processes_count);

		int is_con = is_global_state_consistent_for_pred(process_rank,
				processes_count, pred, global_state, NULL, pred_participants);
		if (is_con == 0) {
			LOG_PRINT("C%i: advance_automaton, not consistent", process_rank);
			continue;
		}
		if (global_state_satisfies_pred(global_state, pred, NULL, process_rank,
				processes_count) == 1) {
			LOG_PRINT("C%i: advance_automaton, new monitor state:%s",
					process_rank, tran.to);
			return get_automaton_state_by_name(tran.to, g_automaton);

		} else {
			LOG_PRINT(
					"C%i: advance_automaton, transition from %s to %s not satisfied",
					process_rank, tran.from, tran.to);

		}
	}
	LOG_PRINT(
			"C%i: advance_automaton, NO satisfying predicate FOUND, stay where we are :(",
			process_rank);
...
int could_be_satisfied_by(GlobalState* global_state, Predicate* pred,
		...
		int processes_count, int*automaton_params, int params_count) {

	int sat = global_state_satisfies_pred(global_state, pred,
	...
	int is_con = is_global_state_consistent_for_pred(process_rank,
			processes_count, pred, global_state, inconsistent,
			pred_participants);
	...
	LOG_PRINT("C%i: could_be_satisfied_by, sat:%i, is_con:%i, unsat[%i]=%i",
			process_rank, sat, is_con, process_rank,
			unsatisfying_processes_boolean_array[process_rank]);
	if (sat == 1 && is_con == 1) {
		LOG_PRINT(
				"C%i: could_be_satisfied_by...weird, global state satisfies the pred",
				process_rank);
		return 1;
	}
		...
		LOG_PRINT(
				"C%i: local state can satisfy predicate, returning true...great",
				process_rank);
		return 1;
	} else {
		LOG_PRINT("C%i: could_be_satisfied_by:false sat:%i, is_con:%i",
				process_rank, sat, is_con);
		return 0;
	}
}
...
int global_state_satisfies_pred(GlobalState* global_state, Predicate* pred,
		...
		int processes_count) {
	int satisfy = 1;
	VariableValuation* variables_array = malloc(
			sizeof(VariableValuation) * processes_count);
	int i;
	for (i = 0; i < processes_count; i++) {
		LOG_PRINT(
				"C%i: global_state->processes_states_array[%i]->variables.state_name_array[0]=%s",
				process_rank, i,
	...
	char* predicate = malloc(
			sizeof(char) * MAX_CONJUNCTS_PER_PREDICATE * MAX_CONJUCATE_LEN);

	predicate[0] = '\0';
	for (i = 0; i < pred->conjunct_count; i++) {
		strcat(predicate, pred->conjuncts[i].conjunct_name);
	}
	LOG_PRINT("C%i:global_state_satisfies_pred checking predicate:%s",
			process_rank, predicate);

	for (i = 0; i < pred->conjunct_count; i++) {

		LOG_PRINT("C%i:global_state_satisfies_pred checking:%s", process_rank,
				pred->conjuncts[i].conjunct_name);

		if (satisfies(&(pred->conjuncts[i]),
				&(variables_array[pred->conjuncts[i].owner_processes[0]]),
...
int token_satisfies_pred(SendableToken *t, int process_rank) {
	...
	for (j = 0; j < t->predicates_count; j++) {
		...
		for (i = 0; i < t->predicates[j].conjunct_count; i++) {
			...
			if (0
					== satisfies(&(t->predicates[j].conjuncts[i]),
							&(t->target_process_variables), NULL,
							process_rank)) {
				pred_sat = 0;
				LOG_PRINT(
						"C%i: token_satisfies_pred: token doesn't satisfy conjunct:%s",
						process_rank,
				...
				LOG_PRINT(
						"C%i: token_satisfies_pred: token satisfies conjunct:%s",
						process_rank,
						t->predicates[j].conjuncts[i].conjunct_name);
...
int is_consistent3(GlobalState* global_state, int processes_count,
		int * inconsistent_processes_boolean_array, int my_rank) {
	int is_con = 1;
	// 1 -1 with me first
	...
	for (i = 0; i < processes_count; i++) {
		...
		vector_clock_compare res = compare_clocks(
				global_state->processes_states_array[i]->vector_clock,
				global_state->processes_states_array[my_rank]->vector_clock,
				processes_count);
		if (res != VC_CONCURRENT && res != VC_EQUAL) {
			is_con = 0;
		...
		for (j = i + 1; j < processes_count; j++) {
			...
			if (res != VC_CONCURRENT && res != VC_EQUAL) {
				...
				if (inconsistent_processes_boolean_array != NULL) {
					...
					if (inconsistent_processes_boolean_array[i] == 0
							&& inconsistent_processes_boolean_array[j] == 0)
						LOG_PRINT("C%i: C%i is inconsistent", my_rank, i);
					inconsistent_processes_boolean_array[i] =
							inconsistent_processes_boolean_array[j] = 1;
				}
			}
		}
...
int is_consistent2(GlobalState* global_state, int processes_count,
		int * inconsistent_processes_boolean_array, int my_rank) {
	int is_con = 1;
	int i;
	...
	for (i = 0; i < processes_count; i++) {
		...
		for (j = 0; j < processes_count; j++) {
			if (i != j) {
				if (global_state->processes_states_array[j]->vector_clock[i]
						> i_value) {
					if (inconsistent_processes_boolean_array != NULL)
						inconsistent_processes_boolean_array[i] = 1;
					is_con = 0;
		...
		if (inconsistent_processes_boolean_array != NULL
				&& inconsistent_processes_boolean_array[i] == 0) {
			LOG_PRINT("C%i: C%i is consistent", my_rank, i);
		}
	}
	return is_con;
}
int is_consistent4(GlobalState* global_state, int processes_count,
		int * inconsistent_processes_boolean_array, int my_rank) {
	int is_con = 1;
	int i;
	...
	for (i = 0; i < processes_count; i++) {
		...
		int event_clock[2];
		event_clock[0] =
				global_state->processes_states_array[my_rank]->vector_clock[my_rank];
		event_clock[1] =
				global_state->processes_states_array[my_rank]->vector_clock[i];
...
int is_consistent(GlobalState* global_state, int processes_count,
		int * inconsistent_processes_boolean_array, int my_rank) {
	int is_con = 1;
	int i;
