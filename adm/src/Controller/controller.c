...
void init_controller(char* initial_state_file, char* automaton_file,
		char* conjunct_mapping_file, int p_count, int rank,
		int initial_global_states_count,
		int** automaton_parameters_per_initial_global_state, int param_count) {
	...
	global_states_array = malloc(
	MAX_GLOBAL_STATES_COUNT * sizeof(GlobalState*));
	global_states_count = 0;
	//array for local event history for current process
	local_state_history = malloc(
			1000 * processes_count * sizeof(ProcessState*));
	g_local_state_history_count = 0;
	// Array for waiting tokens from other processes
	g_waiting_tokens = malloc(
	MAX_WAITING_TOKENS_COUNT * sizeof(SendableToken*));
	...
	load_automaton(automaton_file, &g_automaton, my_rank);

	//Get automaton initial state q_0
	AutomatonState* q_0 = get_initial_automaton_state(&g_automaton);
	LOG_PRINT("C%i: Initialization complete, initial monitor state:%s", my_rank,
			q_0->state_name);
	...
	for (i = 0; i < initial_global_states_count; i++) {
		...
		global_state->automaton_params =
				automaton_parameters_per_initial_global_state[i];
		...
		global_state->processes_states_array = copy_processes_states_deep(
				initial_processes_states_array, processes_count);

		add_new_global_state(global_state);
		AutomatonState* new_automaton_state = advance_automaton(global_state,
				q_0->state_name, processes_count, my_rank, &g_automaton,
				automaton_parameters_per_initial_global_state[i], param_count);
		if (new_automaton_state != NULL) {
			LOG_PRINT(
					"C%i: Initial global state satisfies a transition, new monitor state:%s",
					my_rank, new_automaton_state->state_name);
...
GlobalState* init_global_state() {
	...
	global_state->pending_events = malloc(
			sizeof(int) * MAX_PENDING_EVENTS_COUNT);
...
void start_controller() {
	...
	while (1) {
		...
		MPI_Iprobe( MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD,
				&message_pending, &pending_status);
		...
		if (message_pending == 1) {
			...
			if (pending_status.MPI_TAG == 1) // message from my process (event update)
				...
				MPI_Recv(&recvMsg, sizeof(LocalEvent), MPI_CHAR,
				...
				ProcessState* local_state = convert_local_event_to_local_state(
						&local_event);
				LOG_PRINT(
						"C%i: Received an event: local event # %i from %i, p%i_goingwest:%i",
						my_rank, local_event.event_id,
						pending_status.MPI_SOURCE, my_rank,
				...
				ProcessState* local_state = convert_local_event_to_local_state(
						&local_event);
				LOG_PRINT(
						"C%i: Received an event: communication event # %i from %i, p%i_goingwest:%i",
						my_rank, local_event.event_id,
						pending_status.MPI_SOURCE, my_rank,
						local_event.variables.state_value_array[0]);
				...
				receive_event(local_state);
					...
					{
				MPI_Status request;
				LOG_PRINT("C%i: Received a token from:C%i, size of struct:%i",
						my_rank, pending_status.MPI_SOURCE - (processes_count),
						sizeof(SendableToken));
				char recvMsg[sizeof(SendableToken)];
				MPI_Recv(&recvMsg, sizeof(SendableToken), MPI_CHAR,
						pending_status.MPI_SOURCE, pending_status.MPI_TAG,
						MPI_COMM_WORLD, &request);
				...
				if (received_token->destination != my_rank
						&& received_token->parent_process_rank != my_rank) {
					LOG_PRINT(
							...
							received_token->destination,
							received_token->parent_process_rank,
				...
				LOG_PRINT(
						"C%i: Received a token from C%i: parent:C%i, token ID:%i",
						my_rank, (pending_status.MPI_SOURCE - processes_count),
						received_token->parent_process_rank,
						received_token->unique_id);
				receive_token(received_token);
					...
					{
				MPI_Status request;
				LOG_PRINT("C%i: Received bulk tokens from:C%i", my_rank,
						pending_status.MPI_SOURCE);
				char recvMsg[sizeof(SendableToken) * 50];
				MPI_Recv(&recvMsg, sizeof(SendableToken) * 50, MPI_CHAR,
						pending_status.MPI_SOURCE, pending_status.MPI_TAG,
						MPI_COMM_WORLD, &request);
				int i;
				int count;
				...
				SendableToken* received_tokens = malloc(
						sizeof(SendableToken) * count); //Re-make the struct

				memcpy(received_tokens, recvMsg, sizeof(SendableToken) * count);
				LOG_PRINT("C%i: Received %i bulk tokens from C%i", my_rank,
						count, (pending_status.MPI_SOURCE - processes_count));
				int k;
				int count_bulk_received = 0;

				for (i = 0; i < count; i++) {
					LOG_PRINT(
							"C%i: receive_bulk from C%i:parent:C%i destination:%i,unique ID:%i",
							my_rank,
							(pending_status.MPI_SOURCE - processes_count),
							received_tokens[i].parent_process_rank,
							received_tokens[i].destination,
							received_tokens[i].unique_id);
				}
					...
					if (received_tokens[k].destination != -1) {
						if (received_tokens[k].target_process_rank == my_rank) {
							LOG_PRINT(
									...
									my_rank,
									(pending_status.MPI_SOURCE - processes_count),
									received_tokens[k].parent_process_rank,
				...
				LOG_PRINT_RESULT("%i,[prog-over]Messages sent,%i", my_rank,
						messages_sent);
				LOG_PRINT_RESULT("%i,all Events received,%i", my_rank,
						events_received);
				terminated_controllers[my_rank] = 1;
					...
					{
				int controller_id = pending_status.MPI_SOURCE
						- (processes_count);
				terminated_controllers[controller_id] = 1;
				MPI_Status request;
				LOG_PRINT(
						"C%i: Received an end of program signal from controller:C%i",
						my_rank, (pending_status.MPI_SOURCE - processes_count));
				char recvMsg[5];
				MPI_Recv(&recvMsg, 5, MPI_CHAR, pending_status.MPI_SOURCE,
						pending_status.MPI_TAG, MPI_COMM_WORLD, &request);
				...
				if (informed_others_about_termination == 1) {
					...
					for (i = 0; i < processes_count; i++) {
						if (terminated_controllers[i] == 0) {
							all_done = 0;
							LOG_PRINT(
									"C%i: controller C%i is not terminated yet",
									my_rank, i);
							break;
						}
					}
			...
			if (all_done == 1) {
				...
				if (my_rank == 0) {
					...
					if (first_bulk_send
							== 0&& bulk_tokens_request != MPI_REQUEST_NULL) {
			...
			if (terminated_controllers[my_rank] == 1) {
				...
				for (i = 0; i < g_waiting_tokens_count; i++) {
					if (g_waiting_tokens[i]->target_event_id
							>= g_local_state_history_count - 1) {
						LOG_PRINT(
								"C%i: waiting token for further events... returning waiting token to C%i",
								my_rank,
								g_waiting_tokens[i]->parent_process_rank);
						int j;
						for (j = 0; j < g_waiting_tokens[i]->predicates_count;
								j++) {
							g_waiting_tokens[i]->predicates_eval[j] = 0;
						}
						g_waiting_tokens[i]->gcut[my_rank] =
								local_state_history[g_local_state_history_count
										- 1]->vector_clock[my_rank];
						g_waiting_tokens[i]->target_process_variables =
								local_state_history[g_local_state_history_count
										- 1]->variables;
						g_waiting_tokens[i]->target_process_rank =
								g_waiting_tokens[i]->parent_process_rank;
						send_token(g_waiting_tokens[i],
								g_waiting_tokens[i]->parent_process_rank);
				...
				if (pending_events_exists == 0) //if I have no pending events and the app terminated, return all waiting tokens
					...
					LOG_PRINT("C%i: waiting tokens count:%i", my_rank,
							g_waiting_tokens_count);
					for (i = 0; i < g_waiting_tokens_count; i++) {
						LOG_PRINT(
								"C%i: Processed all events... returning waiting token to C%i",
								my_rank,
								g_waiting_tokens[i]->parent_process_rank);
				...
				if (pending_events_exists == 0
					...
					LOG_PRINT(
							"C%i: Will send termination signal to other controllers",
							my_rank);
					int i;
					for (i = 0; i < processes_count; i++) {
						if (i != my_rank) {
							...
							MPI_Isend(&over, sizeof(over), MPI_CHAR,
									controller_id, 5,
									MPI_COMM_WORLD, &bulk_tokens_request);
							LOG_PRINT("C%i: sent termination signal to c%i",
									my_rank, i);
	...
	if (signaled == 0) {
		LOG_PRINT_RESULT("%i,Messages sent- finished,%i", my_rank,
				messages_sent);
		LOG_PRINT_RESULT("%i,Events received- finished,%i", my_rank,
				events_received);
	} else {
		LOG_PRINT_RESULT("%i,[All done]Messages sent- finished,%i", my_rank,
				messages_sent);
		LOG_PRINT_RESULT("%i,[All done]Events received- finished,%i", my_rank,
				events_received);
	}

	LOG_PRINT_RESULT("%i,max global states count,%i", my_rank,
			global_states_ids_count);

	int i;
	for (i = 0; i < global_states_count; i++) {
		LOG_PRINT_RESULT("%i,Global State,%i,%s", my_rank,
				global_states_array[i]->unique_id,
	...
	for (i = 0; i < global_states_count; i++) {
		...
		if (s->tokens_count == 0) {
			if (s->pending_events_count != 0) {
				LOG_PRINT(
						"C%i: process_pending_events_all pending events count:%i, calling process_pending_events",
...
void process_waiting_tokens(ProcessState* local_state) {
	...
	for (i = 0; i < g_waiting_tokens_count; i++) {
		...
		if (g_waiting_tokens[i]->target_event_id
			...
			LOG_PRINT(
					"C%i: receive_event: processing waiting_token:%i with target_event_id:%i",
					my_rank, waiting_token->unique_id,
					waiting_token->target_event_id);
			...
			LOG_PRINT(
					"C%i: receive_event: received event:%i doesn't match: waiting_token: %i target_event_id=%i",
					my_rank, local_state->vector_clock[my_rank],
					g_waiting_tokens[i]->unique_id,
...
void receive_event(ProcessState* local_state) {
	LOG_PRINT("C%i: receive_event: adding event to local event history",
			my_rank);
	...
	LOG_PRINT(
			"C%i: receive_event: checking waiting token, waiting tokens count:%i",
			my_rank, g_waiting_tokens_count);

	LOG_PRINT("C%i: receive_event: new waiting tokens count:%i", my_rank,
			g_waiting_tokens_count);

	LOG_PRINT(
			"C%i: receive_event: checking global states, global states count:%i",
			my_rank, global_states_count);
	...
	for (i = 0; i < global_states_count; i++) {
		...
		LOG_PRINT(
				"C%i: receive_event: checking global state[%i], state:%s tokens count:%i",
				my_rank, i,
				global_states_array[i]->current_monitor_state->state_name,
				s->tokens_count);
		...
		for (j = 0; j < s->tokens_count; j++) {
			...
			if (t->eval == -1) {
				LOG_PRINT(
						...
						my_rank, i, j, t->unique_id, t->eval, t->transition_id,
						t->target_event_id, t->destination,
						s->pending_events_count);
		...
		if (s->tokens_count == 0) {
			if (s->pending_events_count != 0) {
				LOG_PRINT(
						"C%i: receive_event: pending events count:%i, calling process_pending_events",
						my_rank, s->pending_events_count);
				s->pending_events[s->pending_events_count] =
						g_local_state_history_count - 1;	//last event
				s->pending_events_count++;
				process_pending_events(s);

			} else {
				LOG_PRINT(
						"C%i: receive_event: no waiting tokens at other processes, calling process_event",
						my_rank);
...
void advance_automaton_localy(GlobalState * global_state) {
	AutomatonState* new_state = advance_automaton(global_state,
			global_state->current_monitor_state->state_name, processes_count,
			my_rank, &g_automaton, global_state->automaton_params,
			global_state->params_count);
	if (new_state != NULL) {
		LOG_PRINT(
				"C%i: current global state satisfies a transition, advancing monitor state to %s!",
				my_rank, new_state->state_name);
...
void process_event(GlobalState* global_state, ProcessState* local_state) {
	...
	LOG_PRINT("C%i: processing event#:%i for global_state:%i, p%i_goingwest:%i",
			my_rank, local_state->state_id, global_state->unique_id, my_rank,
			local_state->variables.state_value_array[0]);
...
ProcessState** copy_processes_states_deep(ProcessState** source,
		int processes_count) {

	ProcessState** destination = malloc(
			processes_count * sizeof(ProcessState*));
	int i;
	for (i = 0; i < processes_count; i++) {
...
void log_process_variables(ProcessState** global_state, int processes_count) {
	...
	for (i = 0; i < processes_count; i++) {
		...
		for (j = 0; j < global_state[i]->variables.count; j++) {
			LOG_PRINT("C%i:P%i: %s=%d ", my_rank, i,
					global_state[i]->variables.state_name_array[j],
...
Predicate* parse_predicate_by_process(Predicate* pred, int process_rank) {
	...
	for (i = 0; i < pred->conjunct_count; i++) {
		if (pred->conjuncts[i].owner_processes[0] == process_rank) {
			strcpy(new_pred->conjuncts[new_pred->conjunct_count].conjunct_name,
					pred->conjuncts[i].conjunct_name);
			new_pred->conjuncts[new_pred->conjunct_count].owner_processes[0] =
					pred->conjuncts[i].owner_processes[0];
...
int is_predicates_equal(Predicate* pred1, Predicate* pred2) {
	...
	for (i = 0; i < pred1->conjunct_count; i++) {
		...
		for (j = 0; j < pred2->conjunct_count; j++) {
			if (strcmp(pred1->conjuncts[i].conjunct_name,
					pred2->conjuncts[j].conjunct_name) == 0) {
				found = 1;
			}
		}
		if (found == 0) {
...
void check_outgoing_transitions(GlobalState* global_state, int*vector_clock) {
	...
	for (i = 0; i < g_automaton.transitions_count; i++) {
		...
		if (strcmp(tran.from, global_state->current_monitor_state->state_name)
				!= 0 || strcmp(tran.from, tran.to) == 0) //ignore self loop as well
				{
			continue;
		}
		LOG_PRINT(
				"C%i: check_outgoing_transitions: checking: transition:%i %s to %s",
				my_rank, tran.id, tran.from, tran.to);
		int* unsatisfying_processes_boolean_array = malloc(
				sizeof(int) * processes_count);
		int k;
		for (k = 0; k < processes_count; k++) {
			unsatisfying_processes_boolean_array[k] = 0;
		}
		Predicate* pred = deparametrize_pred(&(tran.pred),
				global_state->automaton_params, global_state->params_count,
				my_rank);

		if (could_be_satisfied_by(global_state, pred,
				unsatisfying_processes_boolean_array, my_rank, processes_count,
				global_state->automaton_params, global_state->params_count)
				== 1) {
			LOG_PRINT(
					"C%i: check_outgoing_transitions: transition:%i could_be_satisfied_by returned true",
					my_rank, tran.id);
			int j;
			for (j = 0; j < processes_count; j++) {
				if (j == my_rank
						|| unsatisfying_processes_boolean_array[j] == 0) {
					continue;
				}
				Predicate* parsed_pred = parse_predicate_by_process(pred, j);
				LOG_PRINT(
						"C%i: process %i: should be checked for transition:%i predicate labelling %s to %s:",
						my_rank, j, tran.id, tran.from, tran.to);
				...
				for (k = 0; k < pending_tokens_for_sending_count; k++) {
					if ((pending_tokens_for_sending[k]->destination == j
							&& pending_tokens_for_sending[k]->target_process_rank
									== j)
							&& (pending_tokens_for_sending[k]->parent_process_rank
									== my_rank)
							&& (global_state->unique_id
									== pending_tokens_for_sending[k]->owner_globalstate_id)
							&& (pending_tokens_for_sending[k]->target_event_id
									== target_event_id)) {
						// check same gcut
						token_to_same_process_found = 1;
						int l;
						for (l = 0; l < processes_count; l++) {
							if (pending_tokens_for_sending[k]->gcut[l]
									!= vector_clock[l])	//global_state->gcut[l])//
									{
						...
						if (token_to_same_process_found == 1) // add my parsed pred to token.predicates array.
							...
							LOG_PRINT(
									"C%i: Found a similar token to C%i, id:%i, use it.",
									my_rank,
									pending_tokens_for_sending[k]->target_process_rank,
							...
							for (m = 0;
									...
									m++) {
								if (is_predicates_equal(parsed_pred,
										&(pending_tokens_for_sending[k]->predicates[m]))
										== 1) {
									LOG_PRINT(
											"C%i: also Found my parsed pred in the token, id:%i, use it.",
											my_rank,
											pending_tokens_for_sending[k]->unique_id);
									pred_found = 1;
							...
							if (pred_found == 0) {
								...
								pending_tokens_for_sending[k]->predicates_eval[pending_tokens_for_sending[k]->predicates_count] =
										-1;
								pred_index_in_sendabletoken =
										pending_tokens_for_sending[k]->predicates_count;
								pending_tokens_for_sending[k]->predicates_count++;
								LOG_PRINT("C%i: token has now %i predicates!",
										my_rank,
										pending_tokens_for_sending[k]->predicates_count);
							}
							...
							add_token_to_global_state(global_state, &tran, pred,
									j, pending_tokens_for_sending[k]->unique_id,
									pred_index_in_sendabletoken,
									target_event_id);
							break;
						}
				...
				if (token_to_same_process_found == 0
						|| pending_tokens_for_sending_count == 0) {
					SendableToken* sendableToken = malloc(
							sizeof(SendableToken));
					...
					sendableToken->owner_globalstate_id =
							global_state->unique_id;
					...
					pending_tokens_for_sending[pending_tokens_for_sending_count] =
							sendableToken;
					pending_tokens_for_sending_count++;
					add_token_to_global_state(global_state, &tran, pred, j,
							token_ids_count, 0, target_event_id);
					LOG_PRINT(
							"C%i: Creating a token to C%i, id:%i targeting event: %i",
							my_rank, j, sendableToken->unique_id,
							sendableToken->target_event_id);

					token_ids_count++;
				}
			}
		} else {
			LOG_PRINT("C%i: tran:%i Could be satisfied returned false", my_rank,
					tran.id);
		}
	}
...
void add_token_to_global_state(GlobalState* global_state,
		AutomatonTransition* tran, Predicate* pred, int j, int token_unique_id,
		int pred_index_in_sendabletoken, int target_event_id) {
	global_state->tokens[global_state->tokens_count] = malloc(sizeof(Token));
	global_state->tokens[global_state->tokens_count]->eval = -1;
	strcpy(global_state->tokens[global_state->tokens_count]->from_state,
			tran->from);
	strcpy(global_state->tokens[global_state->tokens_count]->to_state,
			tran->to);
	...
	global_state->tokens[global_state->tokens_count]->target_event_id =
			target_event_id;
	global_state->tokens[global_state->tokens_count]->start_timestamp =
			global_state->processes_states_array[my_rank]->timestamp;
	global_state->tokens[global_state->tokens_count]->pred_index_in_sendabletoken =
			pred_index_in_sendabletoken;
	global_state->tokens[global_state->tokens_count]->parent_process_rank =
			my_rank;
	global_state->tokens[global_state->tokens_count]->unique_id =
			token_unique_id;
...
void remove_pending_token_for_sending(int j) {
	LOG_PRINT(
			"C%i: remove_pending_token_for_sending: removing   token id:%i, target process:%i of index:%i",
			my_rank, pending_tokens_for_sending[j]->unique_id,
			pending_tokens_for_sending[j]->target_process_rank, j);
...
void send_pending_tokens_bulk() {
	...
	while (flag == 1) {
		...
		LOG_PRINT(
				"C%i: send_tokens_bulk, I have %i tokens to send for this step",
				my_rank, pending_tokens_for_sending_count);

		LOG_PRINT("C%i: send_token_bulk, preparing for C%i", my_rank, j);
		SendableToken** tokens_per_process = malloc(
				sizeof(SendableToken*) * 10);
		...
		for (i = 0; i < pending_tokens_for_sending_count; i++) {
			if (pending_tokens_for_sending[i]->target_process_rank == j) {
				LOG_PRINT("C%i: adding t.id:%i, t.targetprocessrank:%i",
						my_rank, pending_tokens_for_sending[i]->unique_id,
						pending_tokens_for_sending[i]->target_process_rank);
				SendableToken* t = (pending_tokens_for_sending[i]);

				if (tokens_per_process_count >= 10) {
					LOG_PRINT(
							"C%i: send_tokens_bulk, exceeding 50 tokens per process.",
							my_rank);
				...
				LOG_PRINT("C%i: New count:%i. i:%i", my_rank,
						pending_tokens_for_sending_count, i);

			}
		}

		if (tokens_per_process_count != 0) {
			LOG_PRINT("C%i: token per process ptr:%i, pred count:%i,pred[0]:%s",
					my_rank, tokens_per_process,
...
void send_token_bulk(SendableToken** tokens_ptr, int receiver_rank, int count) {
	LOG_PRINT("C%i: send_token_bulk to C%i, count:%i", my_rank, receiver_rank,
			count);
	messages_sent++;
	...
	for (i = 0; i < count; i++) {
		...
		LOG_PRINT(
				"C%i: send_token_bulk:sending to:%i , destination:%i,unique ID:%i, owner:%i",
				my_rank, (receiver_rank), tokens[i].target_process_rank,
				tokens[i].unique_id, tokens[i].parent_process_rank);
	...
	MPI_Isend(&b, sizeof(SendableToken) * count, MPI_CHAR,
			(receiver_rank + processes_count), 7, MPI_COMM_WORLD,
			&bulk_tokens_request);
...
int get_global_state_by_token_id(int token_id, GlobalState** global_states,
		Token** tokens) {
	...
	for (i = 0; i < global_states_count; i++) {
		...
		if (s == NULL || s->tokens == NULL) {
			LOG_PRINT(
					"C%i: get_global_state_by_token_id ERROR global state NULL, gs count: %i",
					my_rank, global_states_count);
			return 0;
		}
	...
	if (count == 0) {
		LOG_PRINT("C%i: get_global_state_by_token_id NOT FOUNDDDD!!!!",
				my_rank);
	}
	return count;
}
void check_new_state(GlobalState* new_global_state, Token* t,
		GlobalState* owner_global_state, int delay_time, int delay_events) {
	...
	int i;
	int found = 0;
	LOG_PRINT_RESULT("%i,New Global State,%s,%i", my_rank,
			new_global_state->current_monitor_state->state_name, transition);
	LOG_PRINT("C%i: New Global State,%s, Transition: %i", my_rank,
			new_global_state->current_monitor_state->state_name, transition);
	if (owner_global_state != NULL && t != NULL) {
		for (i = 0; i < owner_global_state->tokens_count; i++) {
			if (owner_global_state->tokens[i]->transition_id == transition) {
				if (owner_global_state->tokens[i]->eval == 1
						&& (owner_global_state->tokens[i] != NULL)
						&& (owner_global_state->tokens[i]->target_process_variables
								!= NULL)) {
					int p_index = owner_global_state->tokens[i]->destination;
					LOG_PRINT("C%i: check new state: checking token id:%i",
							my_rank, owner_global_state->tokens[i]->unique_id);
					tmp = "x";
					double x_other =
							get_variable_value(
									new_global_state->processes_states_array[p_index]->variables,
									tmp);
					tmp = "y";
					double y_other =
							get_variable_value(
					...
					char p[2000];
					LOG_PRINT(
							"C%i: constructing satisfaction gstate: x:%d, y:%d",
							my_rank, x_other, y_other);
	...
	double x = get_variable_value(
			new_global_state->processes_states_array[my_rank]->variables, tmp);
	tmp = "y";
	...
	signalFinalState(new_global_state,
			new_global_state->current_monitor_state->type, tmp,
			final_state_type, x, y, transition, delay_time, delay_events, all,
			1, index, new_global_state->gcut[my_rank]);
	...
	signalFinalState(new_global_state,
			new_global_state->current_monitor_state->type, tmp,
			final_state_type, x, y, transition, delay_time, delay_events, all,
			0, index, new_global_state->gcut[my_rank]);
...
void merge_similar_global_states() {
	...
	for (i = 0; i < global_states_count; i++) {
		...
		for (j = i + 1; j < global_states_count; j++) {
			if (global_states_array[i]->current_monitor_state
					== global_states_array[j]->current_monitor_state) {
				int k, m;
				if (global_states_array[j]->pending_events_count != 0
						&& arrays_equal(global_states_array[i]->pending_events,
								global_states_array[i]->pending_events_count,
								global_states_array[j]->pending_events,
								global_states_array[j]->pending_events_count)
								== 0) {
					...
					int count = 0;
					for (k = 0;
							k < global_states_array[i]->pending_events_count;
							k++) {
						for (m = 0;
								m < global_states_array[j]->pending_events_count;
								m++) {
							if (global_states_array[i]->pending_events[k]
									== global_states_array[j]->pending_events[m]) {
								pending_events_intersection[count] =
										global_states_array[i]->pending_events[k];
								count++;
							}
						}
					}
					free(global_states_array[i]->pending_events);
					global_states_array[i]->pending_events =
							pending_events_intersection;
				...
				//for(k=0;k<processes_count;k++)
				{
					if (global_states_array[j]->gcut[my_rank]
							!= global_states_array[i]->gcut[my_rank]) {
						merge = 0;
				...
				LOG_PRINT("C%i: Merging global_state[%i] and [%i]", my_rank, i,
						j);
				// merge into i and delete j

				for (k = 0; k < processes_count; k++) {
					if (global_states_array[i]->gcut[k]
							< global_states_array[j]->gcut[k]) {
						global_states_array[i]->gcut[k] =
								global_states_array[j]->gcut[k];
...
int signalFinalState(GlobalState * new_global_state, char* type,
		char* final_state_type, char* name, double x, double y, int transition,
		int delay_time, int delay_events, char* all, int terminate, int i,
		int events_so_far) {
	//send to all other processes informing them of final state
	if (strcmp(type, final_state_type) == 0) {
		LOG_PRINT_RESULT("%i,signal %s,%i,%d,%d,%i,%i,%i%s", my_rank, name, i,
				x, y, transition, delay_time, delay_events, all);
		LOG_PRINT("C%i: signal %s,index:%i,x:%d,y:%d,%i,%i,%i", my_rank, name,
				i, x, y, transition, delay_time, delay_events);

		if (terminate == 1) {
			char over[] = "done";

			LOG_PRINT("C%i: Final signal for Global State:%i", my_rank,
					new_global_state->unique_id);
			//			MPI_Isend(&over, sizeof(over), MPI_CHAR, my_rank, 4,
			//					MPI_COMM_WORLD, &bulk_tokens_request);
...
void receive_token(SendableToken* received_token) {
	LOG_PRINT("C%i: receive_token id:%i, owner:C%i, target_event_id:%i",
			my_rank, received_token->unique_id,
			received_token->parent_process_rank,
			received_token->target_event_id);
	if (received_token->parent_process_rank == my_rank) {
		//get owner global state
		GlobalState ** found_global_states = malloc(
				sizeof(GlobalState*) * MAX_GLOBAL_STATES_COUNT);
		Token** found_tokens = malloc(
				sizeof(Token*) * MAX_TOKENS_PER_GLOBALSTATE_COUNT);
		int count_found = get_global_state_by_token_id(
				received_token->unique_id, found_global_states, found_tokens);
		if (count_found == 0) {
			LOG_PRINT(
					"C%i: receive_token, global state deleted, ignore message",
					my_rank);
			return;
		}
		int k, i;
		for (k = 0; k < count_found; k++) {
			...
			token->eval =
					received_token->predicates_eval[token->pred_index_in_sendabletoken];
			token->end_timestamp = received_token->end_timestamp;
			token->start_timestamp = received_token->start_timestamp;
			token->events_till_evaluation =
					received_token->events_till_evaluation;
			token->target_process_variables = malloc(sizeof(VariableValuation));
			token->target_process_variables->count =
					received_token->target_process_variables.count;
			...
			LOG_PRINT("C%i: sanity check: %d=%d", my_rank,
					token->target_process_variables->state_value_array[0],
					received_token->target_process_variables.state_value_array[0]);

			LOG_PRINT("C%i: receive_token, My token eval:%i", my_rank,
					token->eval);
			//   check tokens for same transitions,
			// If all returned true, then enable transition
			int transition_enabled_bool = check_transition_tokens_state(
					global_state, token);
			int is_consistent = 0;
			...
			LOG_PRINT(
					"C%i: transition %i: %s to %s returns: %i, transition_status:%i",
					my_rank, token->transition_id, token->from_state,
					token->to_state, transition_enabled_bool, tran_status);
			...
			if (transition_enabled_bool == 1 && is_consistent == 1)	//  || (received_token->gcut[destination] - global_state->gcut[destination]) == 1)
				...
				for (i = 0; i < global_state->tokens_count; i++) {
					if (global_state->tokens[i]->transition_id
							== token->transition_id) {
			...
			if (transition_enabled_bool == 1 && is_consistent == 1) {
				...
				new_global_state->gcut = copy_gcut_deep(global_state->gcut,
						processes_count);
				new_global_state->automaton_params =
						global_state->automaton_params;
				new_global_state->params_count = global_state->params_count;
				new_global_state->processes_states_array =
						copy_processes_states_deep(
								global_state->processes_states_array,
								processes_count);//copy_processes_states_deep(global_state->processes_states_array,processes_count);
				copy_token_to_processes_states(received_token, new_global_state,
						processes_count);

				for (i = 0; i < processes_count; i++) {
					LOG_PRINT(
							"C%i: receive_token, new_global_state->gcut[%i]=%i",
							my_rank, i, new_global_state->gcut[i]);
				}
				new_global_state->current_monitor_state =
						get_automaton_state_by_name(token->to_state,
								&g_automaton);
				LOG_PRINT(
						"C%i: receive_token, Transition %i enabled, new global state at %s , processing pending events",
						my_rank, token->transition_id,
						new_global_state->current_monitor_state->state_name);
				add_new_global_state(new_global_state);
				check_new_state(new_global_state, token, global_state,
						delay_time, delay_events);

				new_global_state->pending_events = get_pending_events_deep_copy(
						global_state->pending_events,
						global_state->pending_events_count, processes_count);
				new_global_state->pending_events_count =
						global_state->pending_events_count;
				LOG_PRINT("C%i: receive_token, adding new_global_state",
						my_rank);
			...
			} else if (transition_enabled_bool == 0
					|| (transition_enabled_bool == 1 && is_consistent == 0)) {
				//to disable next
				token->eval = 0;
				...
				if (tran_status == 0) {
					...
					for (i = 0; i < global_state->tokens_count; i++) {
						...
						if ((global_state->tokens[i]->gcut[destination]
								- global_state->gcut[destination]) == 1) {
							if (global_state->gcut[destination]
									< global_state->tokens[i]->gcut[destination]) {
								global_state->gcut[destination] =
										global_state->tokens[i]->gcut[destination];
								//get destination's event details
								global_state->processes_states_array[destination]->variables =
										*(global_state->tokens[i]->target_process_variables);
								global_state->processes_states_array[destination]->vector_clock =
										copy_gcut_deep(
												global_state->tokens[i]->gcut,
												processes_count);
							}
					...
					LOG_PRINT(
							"C%i: receive_token, Transition disabled, global state has more tokens...waiting",
							my_rank);

				} else if (tran_status == 2) {
					// the case where a transition was first enabled, and then later ones were disabled.
					LOG_PRINT(
							"C%i: all tokens returned, some true, some false...deleting global state");
	...
	{
		LOG_PRINT(
				"C%i: receive_token: id:%i token owner: %i, target_event_id:%i",
				my_rank, received_token->unique_id,
				received_token->parent_process_rank,
				received_token->target_event_id);
		...
		if (event == NULL) {
			...
			LOG_PRINT(
					"C%i: receive_token id:%i event requested not yet occurred!, adding to waiting tokens",
					my_rank, received_token->unique_id);
			...
			LOG_PRINT(
					"C%i: receive_token id:%i event requested occurred!, calling process token",
					my_rank, received_token->unique_id);
			...
			if (token_returned == 0) // should be added to waiting tokens, but first check if it targeting old event
				...
				for (i = received_token->target_event_id;
					...
					token_returned = process_token(received_token,
							local_state_history[i], 1);
					if (token_returned == 1) {
						break;
					}
				}
...
void remove_global_state(GlobalState* global_state) {
	...
	LOG_PRINT_TIMELINE(my_rank, "%i,%i,remove_global_state,%i,%i",
			global_state->automaton_params[0],
			global_state->automaton_params[1], global_states_count,
			global_state->unique_id);
	...
	//free(global_state);
	LOG_PRINT("C%i: remove_global_state, current global_states_count:%i",
			my_rank, global_states_count);

}
void remove_waiting_token(int j) {
	LOG_PRINT("C%i: remove_token: removing waiting token id:%i of index:%i",
			my_rank, g_waiting_tokens[j]->unique_id, j);
...
void print_waiting_tokens() {
	int i;
	for (i = 0; i < g_waiting_tokens_count; i++) {
		LOG_PRINT("C%i: waiting_token[%i]:target_event_id:%i, unique_id:%i",
				my_rank, i, g_waiting_tokens[i]->target_event_id,
				g_waiting_tokens[i]->unique_id);
		...
		if (global_state->tokens[i]->eval == 0) {
			...
			for (j = 0; j < global_state->tokens_count; j++) {
				if (i == j)
					continue;
				if (global_state->tokens[i]->transition_id
						== global_state->tokens[j]->transition_id) {
...
int check_transition_tokens_state(GlobalState* global_state, Token* token) {
	int i;
	LOG_PRINT(
			"C%i: check_transition_tokens_state for token %s->%s destination:C%i global state.tokens count: %i",
			my_rank, token->from_state, token->to_state, token->destination,
			global_state->tokens_count);

	for (i = 0; i < global_state->tokens_count; i++) {
		LOG_PRINT(
				"C%i: check_transition_tokens_state:  transition ID:%i token id:%i token eval:%i, destination:C%i",
				my_rank, global_state->tokens[i]->transition_id,
				global_state->tokens[i]->unique_id,
				global_state->tokens[i]->eval,
				global_state->tokens[i]->destination);
...
int check_gcut_consistent(GlobalState *global_state, Token* token) {
	...
	for (j = 0; j < token->pred.conjunct_count; j++) {
		process_participate_in_pred[token->pred.conjuncts[j].owner_processes[0]] =
				1;
	}
	for (i = 0; i < processes_count; i++) {
		if (process_participate_in_pred[i] == 1) {
			int* VC_i = global_state->processes_states_array[i]->vector_clock;
			LOG_PRINT("C%i: check_gcut_consistent, default for C%i ", my_rank,
					i);
			for (k = 0; k < global_state->tokens_count; k++) {
				if (global_state->tokens[k]->transition_id
						== token->transition_id
						&& global_state->tokens[k]->destination == i) {
					VC_i = global_state->tokens[k]->gcut;
					LOG_PRINT(
							"C%i: check_gcut_consistent, found a token for C%i ",
							my_rank, i);
					break;
				}
			}
			for (j = 0; j < processes_count; j++) {
				...
				if (i != j && process_participate_in_pred[j] == 1) {
					int* VC_j =
							global_state->processes_states_array[j]->vector_clock;
					LOG_PRINT("C%i: check_gcut_consistent, default for C%i ",
							my_rank, j);
					for (k = 0; k < global_state->tokens_count; k++) {
						if (global_state->tokens[k]->transition_id
								== token->transition_id
					...
					vector_clock_compare res = compare_clocks(VC_i, VC_j,
							processes_count);
					if (res != VC_CONCURRENT && res != VC_EQUAL) {
						//2 if not concurrent, return 0;
						//LOG_PRINT("C%i: not consistent",my_rank);
						LOG_PRINT(
								"C%i: created gcut is not consistent [c%i, c%i] for tran:%i",
								my_rank, i, j, token->transition_id);
						return 0;

					} else {

						LOG_PRINT(
								"C%i: check_gcut_consistent,C%i and C%i are consistent ",
								my_rank, i, j);
					}
				}
			}
...
void process_pending_events(GlobalState* global_state) {
	...
	LOG_PRINT(
			"C%i: process_pending_events, current process_pending_events_count:%i",
			my_rank, global_state->pending_events_count);
	if (global_state->pending_events_count != 0
			&& global_state->tokens_count == 0) {
		int event_id = dequeue(global_state->pending_events,
				global_state->pending_events_count);
		global_state->pending_events_count = (global_state->pending_events_count
				- 1);
		ProcessState* event = local_state_history[event_id];
		LOG_PRINT(
				"C%i: processing Finally event#:%i for global_state:%i, p%i_goingwest:%i",
				my_rank, event->state_id, global_state->unique_id, my_rank,
				event->variables.state_value_array[0]);
...
void add_new_global_state(GlobalState* new_global_state) {
	...
	global_states_ids_count++;
	LOG_PRINT(
			"C%i: add_new_global_state, params:%i,%i, current global_states_count:%i",
			my_rank, new_global_state->automaton_params[0],
			new_global_state->automaton_params[1], global_states_count);
	LOG_PRINT_TIMELINE(my_rank, "%i,%i,new_global_state,%i,%i",
			new_global_state->automaton_params[0],
			new_global_state->automaton_params[1], global_states_count,
			new_global_state->unique_id);

}

int process_token(SendableToken* t, ProcessState* local_state,
		int already_in_waiting_tokens) {
	LOG_PRINT("C%i: process_token id:%i ", my_rank, t->unique_id);

	LOG_PRINT("C%i: process_token,t->gcut[%i]=%i t->gcut[%i]=%i", my_rank,
			t->parent_process_rank, t->gcut[t->parent_process_rank], my_rank,
			t->gcut[my_rank]);
	LOG_PRINT(
			"C%i: process_token, local_state->vector_clock[%i]=%i, local_state->vector_clock[%i]=%i",
			my_rank, t->parent_process_rank,
			local_state->vector_clock[t->parent_process_rank], my_rank,
			local_state->vector_clock[my_rank]);

	vector_clock_compare res = compare_clocks(t->gcut,
			local_state->vector_clock, processes_count);
	...
	if (res == VC_CONCURRENT || res == VC_EQUAL) {
		...
		LOG_PRINT(
				"C%i: process_token, updating token with local state, variables.count:%i",
				my_rank, local_state->variables.count);
		...
		LOG_PRINT(
				"C%i: process_token id:%i, not concurrent anymore token requested pred failed, return to owner C%i",
...
int evaluate_token(SendableToken* t, ProcessState* local_state,
		int already_in_waiting_tokens) {
	...
	if (token_satisfies_pred(t, my_rank) == 1) //at least one predicate satisfied
			{
		LOG_PRINT(
				"C%i: process_token, token id:%i requested pred success, return to owner C%i",
				my_rank, t->unique_id, t->parent_process_rank);
		...
		t->target_process_rank = t->parent_process_rank;
		t->events_till_evaluation =
				local_state_history[g_local_state_history_count - 1]->state_id
