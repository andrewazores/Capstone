...
void init_controller(char* initial_state_file, char* automaton_file, char* conjunct_mapping_file, int p_count, int rank,
	...
	for (i = 0; i < initial_global_states_count; i++) {
		...
		AutomatonState* new_automaton_state = advance_automaton(global_state, q_0->state_name,
				processes_count, my_rank, &g_automaton, automaton_parameters_per_initial_global_state[i], param_count);
...
void start_controller()
{
	...
	while (1)
	{
		...
		if (message_pending == 1)
		{
			...
			if (pending_status.MPI_TAG == 1) // message from my process (event update)
			{
				...
				LOG_PRINT("C%i: Received an event: local event # %i from %i, p%i_goingwest:%i", my_rank, local_event.event_id,
						pending_status.MPI_SOURCE,my_rank,local_event.variables.state_value_array[0]);
				events_received++;
			...
			else if (pending_status.MPI_TAG == 4) // message from my process (vector clock update)
			{
				...
				memcpy(&local_event, recvMsg, sizeof(local_event));
				ProcessState* local_state = convert_local_event_to_local_state(&local_event);
				LOG_PRINT("C%i: Received an event: communication event # %i from %i, p%i_goingwest:%i", my_rank, local_event.event_id,
						pending_status.MPI_SOURCE,my_rank,local_event.variables.state_value_array[0]);
		...
		else
		{
			...
			if (all_done == 1) {
				...
				if(my_rank==0)
				{
					...
					MPI_Isend(&over, sizeof(over), MPI_CHAR, my_rank, 4,MPI_COMM_WORLD, &bulk_tokens_request);
				}
			}
			...
			if (terminated_controllers[my_rank] == 1)
			{
				...
				for (i = 0; i < g_waiting_tokens_count; i++)
				{
					...
					{
						LOG_PRINT("C%i: waiting token for further events... returning waiting token to C%i",my_rank,g_waiting_tokens[i]->parent_process_rank);
						int j;
						...
						g_waiting_tokens[i]->target_process_variables =
								local_state_history[g_local_state_history_count - 1]->variables;
				...
				if ( pending_events_exists == 0) //if I have no pending events and the app terminated, return all waiting tokens
				{
					...
					for (i = 0; i < g_waiting_tokens_count; i++)
					{
						LOG_PRINT("C%i: Processed all events... returning waiting token to C%i",my_rank,g_waiting_tokens[i]->parent_process_rank);
	...
	{
		LOG_PRINT_RESULT("%i,Global State,%i,%s",my_rank,global_states_array[i]->unique_id,global_states_array[i]->current_monitor_state->state_name);
	}
...
void receive_event(ProcessState* local_state) {
	...
	for (i = 0; i < global_states_count; i++) {
		...
		for (j = 0; j < s->tokens_count; j++)
		{
			...
			Token* t = (s->tokens[j]);
			if (t->eval == -1) {
				LOG_PRINT("C%i: receive_event: token at global_state[%i].token[%i],token_id:%i token eval:%i, transition:%i,target_event_id:%i, target process:%i, s->pending_event_count:%i",
						my_rank, i, j, t->unique_id, t->eval, t->transition_id, t->target_event_id, t->destination,
						s->pending_events_count);
...
void advance_automaton_localy(GlobalState * global_state) {
	AutomatonState* new_state = advance_automaton(global_state,
			global_state->current_monitor_state->state_name, processes_count, my_rank, &g_automaton,
...
void process_event(GlobalState* global_state, ProcessState* local_state) {
	...
	LOG_PRINT("C%i: processing event#:%i for global_state:%i, p%i_goingwest:%i", my_rank, local_state->state_id,global_state->unique_id,my_rank,local_state->variables.state_value_array[0]);
...
void check_outgoing_transitions(GlobalState* global_state, int*vector_clock ) {
	...
	for (i = 0; i < g_automaton.transitions_count; i++) {
		...
		if (could_be_satisfied_by(global_state, pred, unsatisfying_processes_boolean_array,
				my_rank, processes_count, global_state->automaton_params,global_state->params_count) == 1)
		{
				...
				for(k=0;k<pending_tokens_for_sending_count;k++)
				{
							...
							(pending_tokens_for_sending[k]->target_event_id ==target_event_id))
					{
						...
						if(token_to_same_process_found==1) // add my parsed pred to token.predicates array.
						{
							...
							LOG_PRINT("C%i: Found a similar token to C%i, id:%i, use it.",my_rank,pending_tokens_for_sending[k]->target_process_rank, pending_tokens_for_sending[k]->unique_id);

							int m;
							int pred_found=0;
							for(m=0;m<pending_tokens_for_sending[k]->predicates_count;m++)
							{
								if(is_predicates_equal(parsed_pred,&(pending_tokens_for_sending[k]->predicates[m]))==1)
								{
									LOG_PRINT("C%i: also Found my parsed pred in the token, id:%i, use it.",my_rank, pending_tokens_for_sending[k]->unique_id);
									pred_found=1;
							...
							if(pred_found==0)
							{
								pending_tokens_for_sending[k]->predicates[pending_tokens_for_sending[k]->predicates_count]=*parsed_pred;
								pending_tokens_for_sending[k]->predicates_eval[pending_tokens_for_sending[k]->predicates_count]=-1;
								pred_index_in_sendabletoken=pending_tokens_for_sending[k]->predicates_count;
								pending_tokens_for_sending[k]->predicates_count++;
								LOG_PRINT("C%i: token has now %i predicates!",my_rank,pending_tokens_for_sending[k]->predicates_count);
							}
							...
							add_token_to_global_state(global_state,&tran,pred, j,pending_tokens_for_sending[k]->unique_id,pred_index_in_sendabletoken,target_event_id);
							break;
						}
					}
				}
	...
	if (received_token->parent_process_rank == my_rank)
	{
		...
		for (k = 0; k < count_found; k++)
		{
			...
			if ( transition_enabled_bool==1&& is_consistent==1 )//  || (received_token->gcut[destination] - global_state->gcut[destination]) == 1)
			{
				for(i=0;i<global_state->tokens_count;i++)
				{
					if(global_state->tokens[i]->transition_id==token->transition_id)
					{
						...
						{
							global_state->gcut[global_state->tokens[i]->destination] =global_state->tokens[i]->gcut[global_state->tokens[i]->destination];
							//get destination's event details
							global_state->processes_states_array[global_state->tokens[i]->destination]->variables =*( global_state->tokens[i]->target_process_variables);
							global_state->processes_states_array[global_state->tokens[i]->destination]->vector_clock = copy_gcut_deep(global_state->tokens[i]->gcut,
									processes_count);
						}
					}
				}
			...
			if (transition_enabled_bool == 1 && is_consistent==1)
			{
				...
				for (i = 0; i < processes_count; i++) {
					LOG_PRINT("C%i: receive_token, new_global_state->gcut[%i]=%i",
			...
			else if (transition_enabled_bool == 0||(transition_enabled_bool==1 &&is_consistent==0))
			{
				...
				for(i=0;i<global_state->tokens_count;i++)
				{
					if(global_state->tokens[i]->transition_id==token->transition_id &&global_state->tokens[i]->eval==1)
					{
				...
				if (tran_status == 0)
				{
					LOG_PRINT("C%i: receive_token, Transition disabled, no more tokens for this global state, processing pending events",
							my_rank);
					...
					for(i=0;i<global_state->tokens_count;i++)
					{
						...
						if((global_state->tokens[i]->gcut[destination] - global_state->gcut[destination]) == 1)
						{
							if(global_state->gcut[destination]<global_state->tokens[i]->gcut[destination])
							{
								...
								//get destination's event details
								global_state->processes_states_array[destination]->variables =*( global_state->tokens[i]->target_process_variables);
								global_state->processes_states_array[destination]->vector_clock = copy_gcut_deep(global_state->tokens[i]->gcut,
										processes_count);
							}
						}
...
int check_gcut_consistent(GlobalState *global_state,Token* token)
{
	...
	for (i = 0; i < processes_count; i++)
	{
		if(process_participate_in_pred[i]==1)
		{
			...
			{
				if(global_state->tokens[k]->transition_id==token->transition_id && global_state->tokens[k]->destination==i)
				{
			...
			for (j = 0; j < processes_count; j++)
			{
				...
				if (i!=j &&  process_participate_in_pred[j]==1)
				{
					...
					for(k=0;k<global_state->tokens_count;k++)
					{
						if(global_state->tokens[k]->transition_id==token->transition_id && global_state->tokens[k]->destination==j)
						{
					...
					if (res != VC_CONCURRENT && res != VC_EQUAL)
					{
						//2 if not concurrent, return 0;
						//LOG_PRINT("C%i: not consistent",my_rank);
						LOG_PRINT("C%i: created gcut is not consistent [c%i, c%i] for tran:%i", my_rank,
								i, j,token->transition_id);
						return 0;
...
void process_pending_events(GlobalState* global_state) {
	...
	if (global_state->pending_events_count != 0 && global_state->tokens_count == 0) {
		...
		LOG_PRINT("C%i: processing Finally event#:%i for global_state:%i, p%i_goingwest:%i", my_rank, event->state_id,global_state->unique_id,my_rank,event->variables.state_value_array[0]);
