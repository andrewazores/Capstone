...
void init_centralized_controller(char* initial_state_file, char* automaton_file, char* conjunct_mapping_file, int rank,
        int p_count) {
    ...
    while (1) {
        if (*buffer_lock_pointer == 0 && *buffer_size_pointer > 0) {
            ...
            if (local_event->flag == 1) {
                ...
                LOG_PRINT("C%i: Received a local event # %i from %i", my_rank, local_event->event_id,
                        local_event->process_rank);
                LOG_PRINT("C%i: received event:  %s:%i from %i", my_rank, local_event->variables.state_name_array[1],
                        local_event->variables.state_value_array[1], local_event->process_rank);
...
int signalCentralizedFinalState(char* type, char* final_state_type, char* name, double x, double y, int i,
        int terminate) {
