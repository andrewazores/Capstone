...
void log_variablevaluation(VariableValuation* variables_array, int process_rank) {
    ...
    for (i = 0; i < variables_array->count; i++) {
        LOG_PRINT("C%i: %s=%d", process_rank, variables_array->state_name_array[i],
                variables_array->state_value_array[i]);
    }
}
int satisfies(Conjunct* conjunct, VariableValuation* p_first, int* unsatisfying_processes_boolean_array,
        int process_rank) {
    ...
    for (i = 0; i < p_first->count; i++) {
        LOG_PRINT("C%i: satisfies: checking p_first[%i]= %s, against %s", process_rank, i, p_first->state_name_array[i],
            ...
            if (endsWith(conjunct->conjunct_name, tmp)) {
                if (p_first->state_value_array[i] == 0) {
                    ...
                    if (unsatisfying_processes_boolean_array != NULL) {
                        LOG_PRINT("C%i: conjunct:%s, unsatisfied, setting sat of C%i to 0:(", process_rank,
                                conjunct->conjunct_name, conjunct->owner_processes[0]);
