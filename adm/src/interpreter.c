...
void load_variable_process_mapping(char* state_process_mapping_file) {
	...
	fclose(stream);
	g_all_variables_process_mapping = malloc(
			newline_count * sizeof(VariableProcessMapping*));
	stream = fopen(state_process_mapping_file, "r");
	char line[1024];
	char *ptr; //points to the unparsed parts of the string, ignore
	int i = 0;
	while (fgets(line, 1024, stream)) {
		VariableProcessMapping* local_state = malloc(
				sizeof(VariableProcessMapping));
...
int get_process_rank_by_variable(char*variable) {
	...
	for (i = 0; i < g_all_variables_process_mapping_count; i++) {
		if (strcmp(g_all_variables_process_mapping[i]->state_name, variable)
				== 0) {
...
ProcessState** load_initial_state(char* file, int process_count) {
	...
	ProcessState** process_state = malloc(
			process_count * sizeof(ProcessState*));
	int i;
	...
	while (fgets(line, 1024, stream)) {
		...
		strcpy(process_state[process_rank]->variables.state_name_array[count],
				get_field(tmp, 2));
		char *ptr;
		tmp = strdup(line);
...
void load_conjunct_process_mapping(char* file) {
	...
	fclose(stream);
	g_process_conjuncts_mapping = (Conjunct*) malloc(
			newline_count * sizeof(Conjunct));
	stream = fopen(file, "r");
	char line[1024];
	char *ptr; //points to the unparsed parts of the string, ignore
	int i = 0;
...
void log_variablevaluation(VariableValuation* variables_array, int process_rank) {
	...
	for (i = 0; i < variables_array->count; i++) {
		LOG_PRINT("C%i: %s=%d", process_rank,
				variables_array->state_name_array[i],
				variables_array->state_value_array[i]);
	}
}
int satisfies(Conjunct* conjunct, VariableValuation* p_first,
	...
	int i;
	LOG_PRINT("C%i: satisfies: p_first->count:%i", process_rank,
			p_first->count);
	for (i = 0; i < p_first->count; i++) {
		LOG_PRINT("C%i: satisfies: checking p_first[%i]= %s, against %s",
				process_rank, i, p_first->state_name_array[i],
				conjunct->conjunct_name);
		if (startsWith(p_first->state_name_array[i], conjunct->conjunct_name)
				== 1) {
			tmp = "'";	// now check for negation ex: idr1'
			if (endsWith(conjunct->conjunct_name, tmp)) {
				if (p_first->state_value_array[i] == 0) {
					LOG_PRINT("C%i: satisfies: conjunct:%s, satisfied!!",
							process_rank, conjunct->conjunct_name);

					return 1;
				} else {
					if (unsatisfying_processes_boolean_array != NULL) {
						LOG_PRINT(
								"C%i: conjunct:%s, unsatisfied, setting sat of C%i to 0:(",
								process_rank, conjunct->conjunct_name,
								conjunct->owner_processes[0]);

						unsatisfying_processes_boolean_array[conjunct->owner_processes[0]] =
								1;
					}
					return 0;
				}
	...
	LOG_PRINT("C%i: satisfies: conjunct:%s, variable not found!!!",
			process_rank, conjunct->conjunct_name);
	if (unsatisfying_processes_boolean_array != NULL) {
...
void set_unsatisfying_array_for_shared_conjugates(Conjunct* conjunct,
	...
	for (i = 0; i < conjunct->owner_processes_count; i++) {
		if (conjunct->owner_processes[i] != process_rank) {
			unsatisfying_processes_boolean_array[conjunct->owner_processes[i]] =
					1;
