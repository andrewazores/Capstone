...
void log_variablevaluation_process(VariableValuation* variables_array,
		int process_rank);
...
long phi(double x, double mean, double std) {
	...
	w = (w * std) + mean;
	LOG_PRINT("P%i: generated random variable with input:%d mean:%d is %i",
			rank, x, mean, (long )(w));
...
void log_variablevaluation_process(VariableValuation* variables_array,
	...
	for (i = 0; i < variables_array->count; i++) {
		LOG_PRINT("P%i: %s=%d", process_rank,
				variables_array->state_name_array[i],
	...
	for (i = 0; i < checkpoints_count; i++) {
		...
		double distance = sqrt(
				pow((checkpoints[i].x - vehicle_state->location.x), 2)
						+ pow((checkpoints[i].y - vehicle_state->location.y),
								2));
...
//	}
	...
	LOG_PRINT("P%i: new velocity:x:%d, y:%d", rank, vehicle_state->velocity.x,
...
void update_position(long current_timestamp) {
	...
	LOG_PRINT_RESULT("%i,U,%i,%i,%d,%d", rank, current_timestamp,
			iteration_index, vehicle_state->location.x,
...
int main(int argc, char * argv[]) {
	...
	if (rank >= size / 2) {
		...
		int** automaton_parameters = malloc(
				sizeof(int*) * automaton_count_per_process);
		int i;
		...
		for (i = 0; i < automaton_count_per_process; i++) {
			...
			automaton_parameters[i] = malloc(
					sizeof(int) * automaton_param_count);
			int j;
			for (j = 0; j < pcount; j++) {
				automaton_parameters[i][j] = j; //parameters stay the same (property hard coded not parametrized)
			}
		}
		init_controller(initial_state_file, automaton_file,
				conjunct_mapping_file, size / 2, my_process_rank,
				automaton_count_per_process, automaton_parameters,
				automaton_param_count);
		//6- start controller
		start_controller();
		...
		vehicle_state->distance_behind = vehicle_state->distance_front =
				vehicle_state->distance_left = vehicle_state->distance_right =
						initial_distance;
		...
		long next_pos_update = start_time
				+ get_random_number(((double) rand() / (double) RAND_MAX),
						pos_update_mean, variance); //pos_update_period;
		int received_signal = 0;
		char mission_checkpoints_file[1000];
		char mission_checkpoints_file_base[1000] = "%s/%d/mission.txt";
		sprintf(mission_checkpoints_file, mission_checkpoints_file_base,
				exp_folder, rank);
		LOG_PRINT("P%i: loading checkpoints file:%s", rank,
				mission_checkpoints_file);
		checkpoints = load_mission_file(mission_checkpoints_file,
				&checkpoints_count);
		failure_injected_checkpoints = load_mission_file(
				mission_checkpoints_file, &checkpoints_count);
		if (rank == 2) {
			failure_injected_checkpoints[1].x =
					-failure_injected_checkpoints[1].x;
		}
		while (1) {
			...
			if (current_timestamp >= next_pos_update) {
				...
				int updated =
						update_direction(vehicle_state,
								failure_injected_checkpoints[current_checkpoints_index]);
				...
				if (updated == 0) //vehicle in radius of the checkpoint
					...
					next_pos_update = get_time_now()
							+ get_random_number(
									...
									pos_update_mean + 3, variance); //pos_update_period;//phi(rand()%100)//;
					...
					next_pos_update = get_time_now()
							+ get_random_number(
									((double) rand() / (double) RAND_MAX),
									pos_update_mean, variance); //pos_update_period;//phi(rand()%100)//;
				}
			...
			if (flag) {
				...
				if (status.MPI_TAG == 0) {
					...
					MPI_Recv(&recvMsg, sizeof(knowledge_vector), MPI_CHAR,
							status.MPI_SOURCE, 0,
							MPI_COMM_WORLD, &recv_request);
					KnowledgeVector other; //Re-make the struct
					memcpy(&other, recvMsg, sizeof(other));
					LOG_PRINT(
							"P%i: Received message from P%i at iteration:%i: other.x:%d,other.y:%d,",
							rank, status.MPI_SOURCE, iteration_index,
							other.locations[status.MPI_SOURCE].x,
					...
					LOG_PRINT("P%i: Received unknown signal:%i.", rank,
							status.MPI_TAG);
				}
			}
		...
		LOG_PRINT_RESULT("%i,processes_messages,%i", rank,
				process_messages_sent);
