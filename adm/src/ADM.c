...
typedef enum {vector_clock_update, event_update} update_type;
...
void log_variablevaluation_process(VariableValuation* variables_array, int process_rank)
{
	...
	{
		LOG_PRINT("P%i: %s=%d",process_rank,variables_array->state_name_array[i],variables_array->state_value_array[i]);
	}
}
...
LocalEvent* prepare_local_event()
{
	...
	for(i=0;i<checkpoints_count;i++)
	{
		...
		double distance= sqrt(pow((checkpoints[i].x-vehicle_state->location.x),2) + pow((checkpoints[i].y-vehicle_state->location.y),2));
...
void update_position(long current_timestamp)
{
	...
	LOG_PRINT_RESULT("%i,U,%i,%i,%d,%d",rank,current_timestamp,iteration_index,vehicle_state->location.x,vehicle_state->location.y);
...
int main( int argc, char * argv[] )
{
	...
	if(rank>=size/2 )
	{
		...
		init_controller( initial_state_file,
				automaton_file,conjunct_mapping_file,size/2,my_process_rank,
				automaton_count_per_process,
				automaton_parameters,automaton_param_count);
		//6- start controller
		start_controller( );
	...
	else
	{
		...
		srand(rank);
		long next_pos_update=start_time+get_random_number(((double)rand() / (double)RAND_MAX),pos_update_mean,variance);//pos_update_period;
		...
		while(1)
		{
			...
			if(current_timestamp>=next_pos_update)
			{
				...
				{
					// stay here for a while!
					next_pos_update=get_time_now()+get_random_number( ((double)rand() / (double)RAND_MAX),pos_update_mean+3,variance);//pos_update_period;//phi(rand()%100)//;
					current_checkpoints_index++;
				}
				else
				{

					next_pos_update=get_time_now()+get_random_number( ((double)rand() / (double)RAND_MAX),pos_update_mean,variance);//pos_update_period;//phi(rand()%100)//;
				}
			...
			if(flag)
			{
				...
				if(status.MPI_TAG==0)
				{
					...
					LOG_PRINT("P%i: Received message from P%i at iteration:%i: other.x:%d,other.y:%d,"
